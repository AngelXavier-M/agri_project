...
if(isset($register))
{
   ...
   $row=mysqli_fetch_array($q1);
	 ...
	 if($n1==1)
	 {
	$mq=mysqli_query($connect,"select max(id) from ar_fertilizer_booking");
	$mr=mysqli_fetch_array($mq);
	$id=$mr['max(id)']+1;
    $final=$price*$qty;

  
    $ins=mysqli_query($connect,"insert into ar_fertilizer_booking(id,uname,provider,pid,qty,final,req_date,status) values($id,'$uname','$far','$pid','$qty','$final','$req_date','0')");
			if($ins)
			{
		 ?>
	<script language="javascript">
		alert("<?php echo $uname;?>,Your details has Booking!.");
		window.location.href="pay1.php?payid=<?php echo $id;?>";
		 </script>
		 <?php
			}
	}
...
<li>
 ...
 <section id="innerBanner"> 
  <div class="inner-content">
    <h2><span>Farmer</span><br>We provide high quality vehicles</h2>
    <div> 
    </div>
  </div> 
</section><!-- #Page Banner -->
        ...
        <div class="container"> 
          <div class="row">
            ...
            <div class="col-lg-6 content">
              <h4>Booking</h4>
              <?php
                                        $q2 = mysqli_query($connect, "SELECT * FROM ar_provider where uname='$far'");
                                       
                                      $r1 = mysqli_fetch_array($q2);
                                            ?>
			  <p>Provider : <?php echo $r1['name'];?> </p>
