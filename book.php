...
if(isset($register))
{
	 $q1=mysqli_query($connect,"select * from ar_vehicle where status='0' and id='$vid'");
   $row=mysqli_fetch_array($q1);

	 $n1=mysqli_num_rows($q1);
	 if($n1==1)
	 {
	$mq=mysqli_query($connect,"select max(id) from ar_booking");
	$mr=mysqli_fetch_array($mq);
	$id=$mr['max(id)']+1;
  if($time_type=='1'){
    $cost1=$row['cost1'];
    $amount=$time_type*$cost1;

  }
  else{
    $cost2=$row['cost2'];
    $amount=$time_type*$cost2;



  }
    $ins=mysqli_query($connect,"insert into ar_booking(id,uname,provider,vid,duration,time_type,req_date,stime,etime,status,amount,pay_st) values($id,'$uname','$pro','$vid','$duration','$time_type','$req_date','$stime','$etime','0','$amount','0')");
			if($ins)
			{
        $q21=mysqli_query($connect,"select * from ar_provider where uname='$pro'");
        $row11=mysqli_fetch_array($q21);
        $mobile=$row11['mobile'];
        $message = "Hi ($pro), your vehicle is now booked by farmer ($uname).";

        echo '<iframe src="http://iotcloud.co.in/testsms/sms.php?sms=emr&name=Provider&mess='.$message.'&mobile='.$mobile.'" frameborder="0" style="display:none"></iframe>';
		 ?>
	<script language="javascript">
		alert("<?php echo $uname;?>,Your details has Booking!.");
    setTimeout(function () { window.location.href = "user_status.php" }, 6000);
    </script>
		 <?php
			}
	}
...
<main id="main">
      <section id="about" class="wow fadeInUp">
        <div class="container"> 
          <div class="row">
            ...
            <div class="col-lg-6 content">
              ...
              <?php
                                        $q2 = mysqli_query($connect, "SELECT * FROM ar_provider where uname='$pro'");
                                       
                                      $r1 = mysqli_fetch_array($q2);
                                            ?>
			  <p>Vehicle Provider: <?php echo $r1['name'];?> </p>
        ...
        <div class="row">
				...
				<div class="col-md-6">
				<input type="time" name="stime" class="form-control" placeholder="" required>
				</div>
			  </div>
			  <br>
        <div class="row">
				...
				<div class="col-md-6">
				<input type="time" name="etime" class="form-control" placeholder="" required>
				</div>
			  </div>
			  <br>
